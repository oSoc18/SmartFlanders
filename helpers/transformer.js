...
module.exports = async (params) => {
    try {
        ...
        //console.log(match);
        console.log(`https://basisregisters.vlaanderen.be/api/v1/adressen?Postcode=${params.postcode}&Straatnaam=${straat}&Huisnummer=${params.number}`);
        let adresId = await fetch(`https://basisregisters.vlaanderen.be/api/v1/adressen?Postcode=${params.postcode}&Straatnaam=${straat}&Huisnummer=${params.number}`);
        ...
        console.log(JSON.parse(gebouwId));
        return await jsonLD(JSON.parse(gebouwId), JSON.parse(adresId), lambertToWGS(JSON.parse(gebouwId).geometriePunt.point.coordinates[0],JSON.parse(gebouwId).geometriePunt.point.coordinates[1]));
    } catch (err) {
